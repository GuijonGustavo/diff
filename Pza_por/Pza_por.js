...
//d3.tsv("data.tsv", type, function(error, data) {
  x = x.domain(d3.extent(data, function(d) { return d.ieco; })).nice();
  y = y.domain(data.map(function(d) { return d.mun; }));

  svg.selectAll(".bar")
      .data(data)
    .enter().append("rect")
      .attr("class", function(d) { return "bar bar--" + (d.ieco < 0 ? "negative" : "positive"); })
      .attr("x", function(d) { return x(Math.min(0, d.ieco)); })
      .attr("y", function(d) { return y(d.mun); })
      .attr("width", function(d) { return Math.abs(x(d.ieco) - x(0)); })
...
function distQuant(dato, legRow){
	...
	function mouseoverLegend(_,p){	
		...
		if (p ==0 ){	
		updateData("Pza_por/db/Ags.json");}
		if (p ==1 ){	
		updateData("Pza_por/db/Bcn.json");}

		if (p ==2 ){	
		updateData("Pza_por/db/Bcs.json");}
		if (p ==3 ){	
		updateData("Pza_por/db/Cam.json");}

		if (p ==4 ){	
		updateData("Pza_por/db/Chp.json");}
		if (p ==5 ){	
		updateData("Pza_por/db/Chi.json");}
		if (p ==6 ){	
		updateData("Pza_por/db/Dif.json");}
		if (p ==7 ){	
		updateData("Pza_por/db/Coa.json");}
		if (p ==8 ){	
		updateData("Pza_por/db/Col.json");}
		if (p ==9 ){	
		updateData("Pza_por/db/Dur.json");}
		if (p ==10 ){	
		updateData("Pza_por/db/Gto.json");}
		if (p ==11 ){	
		updateData("Pza_por/db/Gro.json");}
		if (p ==12 ){	
		updateData("Pza_por/db/Hgo.json");}
		if (p ==13 ){	
		updateData("Pza_por/db/Jal.json");}
		if (p ==14 ){	
		updateData("Pza_por/db/Mex.json");}
		if (p ==15 ){	
		updateData("Pza_por/db/Mic.json");}
		if (p ==16 ){	
		updateData("Pza_por/db/Mor.json");}
		if (p ==17 ){	
		updateData("Pza_por/db/Nay.json");}
		if (p ==18 ){	
		updateData("Pza_por/db/Nle.json");}
		if (p ==19 ){	
		updateData("Pza_por/db/Oax.json");}
		if (p ==20 ){	
		updateData("Pza_por/db/Pue.json");}
		if (p ==21 ){	
		updateData("Pza_por/db/Qro.json");}
		if (p ==22 ){	
		updateData("Pza_por/db/Roo.json");}
		if (p ==23 ){	
		updateData("Pza_por/db/Slp.json");}
		if (p ==24 ){	
		updateData("Pza_por/db/Sin.json");}
		if (p ==25 ){	
		updateData("Pza_por/db/Son.json");}
		if (p ==26 ){	
		updateData("Pza_por/db/Tab.json");}
		if (p ==27 ){	
		updateData("Pza_por/db/Tam.json");}
		if (p ==28 ){	
		updateData("Pza_por/db/Tlx.json");}
		if (p ==29 ){	
		updateData("Pza_por/db/Ver.json");}
		if (p ==30 ){	
		updateData("Pza_por/db/Yuc.json");}
		if (p ==31 ){	
		updateData("Pza_por/db/Zac.json");}
	}
